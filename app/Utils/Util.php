...
class Util
{
    ...
    */
    ...
    public function uploadFilraw($file, $dir_name)
    {
        //If app environment is demo return null
        
        $uploaded_file_name = null;
        if ($file->isValid()) {
          //  echo  $file->getSize() <= config('constants.document_size_limit');
            if ($file->getSize() <= config('constants.document_size_limit')) {
                $new_file_name = time() . '_' . $file->getClientOriginalName();
                  echo  public_path().$dir_name. $new_file_name;
             //     $file->move(public_path().$dir_name. $new_file_name); 
