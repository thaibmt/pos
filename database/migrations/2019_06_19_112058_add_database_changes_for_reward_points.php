...
class AddDatabaseChangesForRewardPoints extends Migration
{
    ...
    public function up()
    {
        Schema::table('business', function (Blueprint $table) {
            ...
            $table->decimal('amount_for_unit_rp', 20, 2)->default(1)->after('rp_name')->comment('rp is the short form of reward points');
            $table->decimal('min_order_total_for_rp', 20, 2)->default(1)->after('amount_for_unit_rp')->comment('rp is the short form of reward points');
            $table->integer('max_rp_per_order')->nullable()->after('min_order_total_for_rp')->comment('rp is the short form of reward points');

            $table->decimal('redeem_amount_per_unit_rp', 20, 2)->default(1)->after('max_rp_per_order')->comment('rp is the short form of reward points');
            $table->decimal('min_order_total_for_redeem', 20, 2)->default(1)->after('redeem_amount_per_unit_rp')->comment('rp is the short form of reward points');
        ...
        Schema::table('transactions', function (Blueprint $table) {
            ...
            $table->decimal('rp_redeemed_amount', 20, 2)->default(0)->after('rp_redeemed')->comment('rp is the short form of reward points');
        });
